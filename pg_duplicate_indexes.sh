...
echo " \t\tScript Description : This is script to findout duplicate indexes of table in each schema" | tee -a $logs
...
echo " \t****** List of duplicate indexes in each schema\n *******"
$PGHOME/bin/psql -p $PGPORT -d $PGDATABASE -U $PGUSER -c "SELECT indrelid::regclass AS TableName , array_agg(indexrelid::regclass) AS Indexes FROM pg_index GROUP BY indrelid, indkey HAVING COUNT(*) > 1;" | tee -a $logs
end_time
exec_time
