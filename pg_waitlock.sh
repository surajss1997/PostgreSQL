...
echo -e "\t\t\t\t\t      ************************\n"
$PGHOME/bin/psql -p $PGPORT -d $db -c "select pid AS \"PID\", date_trunc('second',query_start::timestamp) AS \"Started Since\", waiting AS \"Waiting\", date_trunc('second',now()::timestamp) - date_trunc('second',query_start::timestamp) AS \"Waiting Since\", substr(query,1) AS \"Query\" from pg_stat_activity where pid<>pg_backend_pid() and waiting='f' and state not in ('idle','active') and now() - query_start > interval '1 minutes' and datname='$db' order by pid limit 25"
...
if [ $op = 1 ]; then
	...
	read yn
		if [ "$yn" = Y -o "$yn" = y ]; then
			$PGHOME/bin/psql -d $db -c "select pg_terminate_backend(pid) from pg_stat_activity where datname='$db' and pid in ($pid)" >/dev/null
