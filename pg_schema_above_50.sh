...
#echo -e "\n$DATABASES"
echo "$DATABASES"
echo -e "\n Enter your database [ex.postgres] : \c "
read db
}
schema_info()
{
echo "list of schema available in $PGADATABASE"
schema=`$PGHOME/bin/psql -p $PGPORT -d $PGDATABASE -U postgres -t -c "select schema_name from information_schema.schemata where schema_name not in ('pg_catalog','information_schema') and schema_name not like 'pg_%' order by schema_name;"`
echo "$schema"
read -p "enter your schema [ex.public] :" sc
}
...
$PGHOME/bin/psql -U $PGUSER -d $PGDATABASE -p $PGPORT -c "$table_query"
...
SELECT dbname as \"Database Name\", nspname as \"Schema Name\", table_name as \"Table Name\", pg_size_pretty(table_bytes) as \"Table Size\", index_name as \"Index Name\",
        ...
        pg_size_pretty(wastedbytes::bigint) as \"Bloat Size\",
                ...
                FROM raw_bloat
WHERE ( realbloat > 50 ) and nspname="$s1" --and wastedbytes > 50000000 )
ORDER BY wastedbytes DESC,nspname"

#$PGHOME/bin/psql -U $PGUSER -d $db -p $PGPORT -c "$index_query"
$PGHOME/bin/psql -U $PGUSER -d $PGDATABASE -p $PGPORT -c "copy($index_query) to '/tmp/$PGDATABASE"_"$sc"_"bloat.csv' with delimiter '|' csv header"
if [ $? -eq 0 ]
then
echo " bloat csv file store at location : /tmp/$PGDATABASE"_"$sc"_"bloat.csv "
else
